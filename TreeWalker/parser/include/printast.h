...
private:
    ...
    std::string ExprAstToString(std::string lexme,  std::vector<ExpressionWrapper>child);
    std::string BlockTraverse(std::vector<StatementPtr>& statements);
    std::string BlockTraverse (std::vector<std::shared_ptr<Statement>> statements);
};
