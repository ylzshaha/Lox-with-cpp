...
Parser::PrimaryExprCreate()
{
    ...
    if(Match(primary_type_list)){
        ObjectType literal_value = Previous()->get_literal();
        unique_ptr<Expression> node = unique_ptr<Expression>(new Literal(literal_value));
        return node;
    }
...
Parser::PrintStatementCreate()
{
    ...
    return unique_ptr<Statement>(new PrintStatement(print_value));
...
Parser::ExpressionStatementCreate()
{
    ...
    Consume(TokenType::SEMICOLON, "Expected ';' after expression.", ErrorType::EXCEPT_SEMICOLON);
    return unique_ptr<Statement>(new ExpressionStatement(value));
}
...
Parser::BlockStatementCreate()
{
    ...
    return unique_ptr<Statement>(new BlockStatement(inner_statements));
}
...
Parser::IfStatementCreate()
{
    ...
    return unique_ptr<Statement>(new IfStatement(if_conditions, if_branches));
}
...
Parser::WhileStatementCreate()
{
    ...
    return unique_ptr<Statement>(new WhileStatement(condition_expr, body_statement));
}
...
Parser::ForStatementCreate()
{
    ...
    if(side_effect != nullptr){
        ...
        statements.push_back(unique_ptr<Statement>(new ExpressionStatement(side_effect)));
        body = unique_ptr<Statement>(new BlockStatement(statements));
    }
    if(condition_expr == nullptr)
        condition_expr = unique_ptr<Expression>(new Literal(bool(true)));
    unique_ptr<Statement> result = unique_ptr<Statement>(new WhileStatement(condition_expr, body));
    if(initial_statement != nullptr){
        vector<unique_ptr<Statement>> statements;
        statements.push_back(move(initial_statement));
        statements.push_back(move(result));
        result = unique_ptr<Statement>(new BlockStatement(statements));
    }
    return result;
...
Parser::FunctionDeclCreate ()
{
    ...
    while (!Check(TokenType::RIGHT_BRACE) && !IsEnd()){
        body.push_back(shared_ptr<Statement>(DeclOrNormalStatementCreate().release()));
    }
    Consume(TokenType::RIGHT_BRACE, "Expect '}' after block.",ErrorType::EXCEPT_RIGHT_BRACE);
    return unique_ptr<Statement>(new FunctionDecl(identifier, parameter_list, body));
}
...
Parser::ClassDeclCreate()
{
    ...
    while(!Check(TokenType::RIGHT_BRACE) && !IsEnd()){
        shared_ptr<FunctionDecl> method = shared_ptr<FunctionDecl>(FunctionDeclCreate().release()->IsFunctionDecl());
